...
from training import ALL_range_evaluation as range_evaluation
...
def _update_parameters(
  ...
  (loss, (metrics, accuracy, debug)), grads = jax.value_and_grad(_apply_loss_and_metrics_fn, has_aux=True)(
      params, rng_key, batch, model_apply_fn, loss_fn, accuracy_fn, task_id, init_para, share_input_size, cl_setting, is_autoregressive, prev_params,
  )
...
class TrainingWorker:
  ...
  def run(
    ...
    rngs_reserve = min(_MAX_RNGS_RESERVE, training_params.training_steps*len(training_params.tasks))
    ...
    for task_seq in range(num_task):
        ...
        #save parameter to use next task
