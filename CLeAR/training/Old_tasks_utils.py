...
def make_model_with_empty_targets(
    ...
    generalization_task: list,# of task.GeneralizationTask,
    computation_steps_mult: int = 0, #! default
    single_output: list = None,
    share_input: bool = False,
    each_input_dim_list:list = None,

) -> Callable[[chex.Array], chex.Array]:
  """Returns a wrapped model that pads the inputs to match the output length. #! seq to seq --> input : seq-0000 (pad behind to make output)
  ...
  def new_model(x: chex.Array, task_id =0, init_para = False, share_input_size: int = 0) -> chex.Array:
    batch_size, input_length, input_size = x.shape

    output_length_list = []
    for task in generalization_task:
      output_length_list.append(task.output_length(input_length))
    max_output_length = max(output_length_list)

    #share input
    if share_input:
      final_input_size = share_input_size
    else:
      final_input_size = each_input_dim_list[task_id]
    ...
    output_tokens = jnp.full(
      ...
      fill_value=final_input_size-1,
    )
    output_tokens = jnn.one_hot(output_tokens, num_classes=final_input_size)


    '''#!
    print('task id', task_id) # 0 (parity_check)
    print(x.shape) #(128, input length, 3)
    print(x)
    print(computation_tape.shape) #(128, 0, 3)
    print(computation_tape) #[]
    print(output_tokens.shape) #(128, 1, 3)
    print(output_tokens) #[[[0. 0. 1.]].....[[[0. 0. 1.]]
    '''
    final_input = jnp.concatenate([x, computation_tape, output_tokens], axis=1) #! concatenate in length dimension
    #! print('final_input',final_input.shape) #! final_input (128, input length +1 , 3)


    if 'input_length' in inspect.getfullargspec(model).args:
      #! @here run CL_rnn
      output = model(final_input, input_length=input_length, task_id = task_id, init_para = init_para, )  # pytype: disable=wrong-keyword-args
    else:
      output = model(final_input)
    #! change output from B*length*output_size to [B*length*output_size for task]
    '''
    ...
    for idx in range(len(output)):
      output[idx] = output[idx][:, input_length*(1+computation_steps_mult):input_length*(1+computation_steps_mult)+output_length_list[idx]]
      if single_output[idx]:
        output[idx] = jnp.squeeze(output[idx], axis=1)
...
def shuffles(tasks_list):
  out = []
  tmp = list(itertools.permutations(tasks_list))
  for item in tmp:
    out.append((False, list(item)))
  out.append((True, tasks_list))
  return out
