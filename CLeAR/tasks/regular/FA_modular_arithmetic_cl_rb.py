...
class ModularArithmetic(task.GeneralizationTask):
    ...
    def __init__(self,
        ...
        self.fa_dim = fa_dim
        self._modulus = 8
        self.setting = modulus
        print('Modular arithmetic with Setting >>>>>', self.setting)
        if operators is None:
            operators = ('+', '*', '-')
        self._operators = (OP_BY_CHARACTER[op] for op in
                           operators)  # ! OP_BY_CHARACTER = {'+': 0, '-': 1, '*': 2, '_': 3}
        self._operators = list(self._operators)  # [0,2,1]
        self.seting_choice = [jnp.array([0,1,2,3]),jnp.array([2,3,4]),jnp.array([3,4,5]),
                              jnp.array([4,5,6,7]),jnp.array([0,2,4,6]),jnp.array([1,3,5,7])]
    ...
    def sample_batch(
        ...
        remainders = jax.random.choice(rng1, self.seting_choice[self.setting],(batch_size, (sequence_length + 1) // 2),)
        #remainders = jax.random.randint(rng1,(batch_size, (sequence_length + 1) // 2), 0,self._modulus)  # _modulus: 5
    ...
    def fa_transform(self, rng, x, sequence_length):
      ...
      5: 1357
        ...
        if self.setting == 0:
            tmp = x*2+rand_add_2
        if self.setting == 1:
            tmp = jnp.where(x < 4, (x-2) * 3 + rand_add_3, 6+rand_add_2)
        if self.setting == 2:
            tmp = jnp.where(x < 5, (x-3) * 3 + rand_add_3, 6+rand_add_2)
        if self.setting == 3:
            tmp = (x-4)*2+rand_add_2
        if self.setting == 4:
            tmp = x+rand_add_2
        if self.setting == 5:
            tmp = x-rand_add_2

        x = jnp.where(x < self._modulus, tmp,jnp.where(x < self._modulus+2, (x-self._modulus) * 3+ rand_add_3, 6 + rand_add_2))
        # '''
