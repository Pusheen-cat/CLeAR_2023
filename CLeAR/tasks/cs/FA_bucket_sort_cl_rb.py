...
class BucketSort(task.GeneralizationTask):
  ...
  def __init__(self, fa_dim: int=3, vocab_size: int = 4, *args,  **kwargs) -> None:
    ...
    self.fa_dim = fa_dim
    print('Vocabl size of BucketSort >>>>>>>>>>', vocab_size)
    self.seting_choicd = [jnp.array([0, 1, 2, 3]), jnp.array([2, 3, 4]), jnp.array([3, 4, 5]),
  ...
  def sample_batch(
    ...
    strings = jax.random.choice(rng, self.seting_choicd[self._vocab_size], shape=(batch_size, length))
    #strings = jrandom.randint( rng, shape=(batch_size, length), minval=0, maxval=self._vocab_size)
  ...
  def output_size(self) -> int:
    ...
    return len(self.seting_choicd[self._vocab_size])
