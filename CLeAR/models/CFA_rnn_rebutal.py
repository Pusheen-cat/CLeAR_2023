...
def make_rnn(
        ...
        share_input_dim=0, #! share_input_dim if share_input else 0,
    ...
    def rnn_model(x: jnp.array, input_length: int = 1 , task_id: int=0, init_para: bool=False) -> jnp.ndarray:
        ...
        output, internal_state = hk.dynamic_unroll(
            core, x, initial_state, time_major=False, return_all_states=True)
        #print(output.shape) #(128=batch, 28=length, 256=out dim)
        '''
        #@stack RNN
        print(len(internal_state)) #2
        print(internal_state[0].shape) #(128, 28=length, 1, 30, 8) #stack
        print(internal_state[1].shape)  # (128, 28=length, 256) #hidden state
        print(internal_state[2].shape) #(128, 28, 1, 3)
        
        #@RNN
        print(internal_state.shape) #(128, 28=length, 256)
        '''
