...
https://flax.readthedocs.io/en/latest/guides/extracting_intermediates.html
"""
...
_StackRnnState = Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray] #third: Stack action
...
class StackRNNCore(hk.RNNCore):
  ...
  def __call__(
    ...
    """
    #print(inputs.shape) #(128=batch, 4)
    stacks, old_core_state, prev_action = prev_state
    #print(stacks.shape) #(128, 1, 30, 8)
    #print(old_core_state.shape) #(128, 256)
    ...
    new_stacks = jax.vmap(
        _update_stack, in_axes=1, out_axes=1)(stacks, stack_actions,
                                              push_values)
    return new_core_output, (new_stacks, new_core_state, stack_actions)
  ...
  def initial_state(self, batch_size: Optional[int]) -> _StackRnnState:
    ...
    actions = jnp.zeros((batch_size, self._n_stacks, 3))
    return stacks, core_state, actions
