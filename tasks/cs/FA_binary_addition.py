...
class BinaryAddition(task.GeneralizationTask):
  ...
  def __init__(self, fa_dim=3):
    self.fa_dim = fa_dim
    assert fa_dim == 4, 'BinaryAddition requires positional column'
  ...
  def sample_batch(
    ...
    one_hot_strings = fa_transform(rng, expressions, length)
...
def fa_transform( rng, x, length):
  # 2 --> 1 features
  #return jnp.expand_dims(x, 2)
  pos, rng1, rng2 = jax.random.split(rng, 3)
  stripe = jnp.where(x == 2, 1, 0)
  # 2 --> 3 features
  x = jnp.where(x<2, x*4+ jrandom.randint(rng1, shape=x.shape, minval=0, maxval=4), jrandom.randint(rng2, shape=x.shape, minval=0, maxval=8))
  x_1 = x % 2
  x = x // 2
  x_2 = x % 2
  x = x // 2
