...
class DivideByTwo(task.GeneralizationTask):
  ...
  def __init__(self, fa_dim=4):
    self.fa_dim = fa_dim
    assert fa_dim == 4, 'DivideByTwo must include positional column'
  ...
  def sample_batch(self, rng: jnp.ndarray, batch_size: int,
    ...
    one_hot_strings = self.fa_transform(rng_trans, input_strings, length)

    output_one_hot = jnn.one_hot(output_strings, num_classes=self.output_size)

    return {"input": one_hot_strings, "task_info": None, "output": output_one_hot}
  ...
  def fa_transform(self, rng, input_strings, length):
    # 2 --> 1 features
    #return jnp.expand_dims(x, 2)
    _, rng= jax.random.split(rng)
    # 2 --> 3 features
    x = jrandom.randint(rng, shape=input_strings.shape, minval=0, maxval=8)
    x_1 = x % 2
    x = x // 2
    x_2 = x % 2
    x = x // 2
    ...
    x = jnp.stack((input_strings, x_1, x_2, x_3), axis=2)
